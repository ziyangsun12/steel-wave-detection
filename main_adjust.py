...
class RegularModeWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.total_frames = 0
        self.is_paused = False  # 跟踪暂停状态
        self.cached_frame = None  # 缓存当前帧用于暂停时处理
    ...
    def on_param_changed(self, slider, label, name, scale):
        ...
        if self.thread and hasattr(self.thread, 'detector'):
            ...
            if self.is_paused and self.cached_frame is not None:
                self.preview_current_frame()

    def preview_current_frame(self):
        """在暂停时预览当前帧的处理效果"""
        if self.thread is None or self.cached_frame is None:
            return
        
        # 使用当前参数处理缓存的帧
        heatmap, status, contour_frame, dehazed_frame = self.thread.detector.process_frame(self.cached_frame)
        
        # 显示轮廓提取的结果
        self.display_image(self.cached_frame, self.video_label)
...
class MainApp(QWidget):
    def __init__(self):
        ...
        self.regular_mode = RegularModeWindow()
        self.debug_mode = MainWindow()
        
        # 将页面添加到堆叠窗口
        self.mode_stack.addWidget(self.home_page)     # index 0
        self.mode_stack.addWidget(self.help_page)     # index 1
        self.mode_stack.addWidget(self.regular_mode)  # index 2
        self.mode_stack.addWidget(self.debug_mode)    # index 3
        
        # 主页信号连接
        self.home_page.mode_selected.connect(self.handle_mode_selection)
        
        # 帮助页面信号连接
        self.help_page.back_to_home.connect(self.show_home)
        
        # 常规模式信号连接
        self.regular_mode.back_to_home.connect(self.show_home)
        self.regular_mode.switch_to_debug.connect(self.show_debug_mode)
        
        # 调试模式信号连接
        self.debug_mode.back_to_home.connect(self.show_home)
        self.debug_mode.switch_to_regular.connect(self.show_regular_mode)
    ...
    def show_regular_mode(self):
        ...
        self.regular_mode.resize(1000, 700)
        self.regular_mode.show()
        # 确保常规模式窗口正确初始化
        if hasattr(self.regular_mode, 'thread') and self.regular_mode.thread is None:
            pass  # 已经初始化过了
    
    def show_debug_mode(self):
        self.mode_stack.setCurrentIndex(3)
        self.debug_mode.resize(1500, 950)
        self.debug_mode.show()
    
    def closeEvent(self, event):
        # 关闭所有线程
        for window in [self.regular_mode, self.debug_mode]:
            if hasattr(window, 'thread') and window.thread is not None and window.thread.isRunning():
                window.thread.stop()
        event.accept()
